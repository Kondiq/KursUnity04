...
public class AIPlayerFinder : MonoBehaviour
{
    ...
    public void GoToPlayerTile(MapManagerComponent.Tile tile)
    {
        ...
        //getobjecttile, check if player in the same tile
    ...
    private bool IsPlayerInFOV(float halfFOV)
    {
        ...
        if (angleToPlayer >= -halfFOV && angleToPlayer <= halfFOV)
        {
            ...
            if (Physics.Raycast(this.transform.position+2*this.transform.forward, mapManager.GetPlayerPosition()+ this.transform.forward, viewDistance, maskPlayer))
            {
    ...
    void Start()
    {
        ...
        maskPlayer = LayerMask.GetMask("Player");
    }
